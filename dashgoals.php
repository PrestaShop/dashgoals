...
class dashgoals extends Module
{
    ...
    public function uninstall()
    {
        ...
        $idConfigurationTab = (int) Tab::getIdFromClassName('AdminDashgoalsConfiguration');
        if ($idConfigurationTab) {
            (new Tab($idConfigurationTab))->delete();
        }
    ...
    {
        if (Tab::getIdFromClassName('AdminDashgoalsConfiguration')) {
            return true;
        }

        $tab = new Tab();
        $tab->active = true;
        $tab->class_name = 'AdminDashgoalsConfiguration';
        $tab->name = [];
        foreach (Language::getLanguages(true) as $lang) {
            $tab->name[$lang['id_lang']] = 'Dashgoals configuration';
        }
        $tab->id_parent = -1;
        $tab->module = $this->name;
    ...
    public function hookDisplayAdminDashboardZoneTwo(array $params)
    {
        ...
        return $this->render('zone_two.html.twig', [
            ...
            'chartConfig' => $this->getSalesGoalChartConfig($chartData['data']),
    ...
    {
        if (!Tab::checkTabRights((int) Tab::getIdFromClassName('AdminDashgoalsConfiguration'))) {
            return null;
        }
